...
- Users whose services are port forwarded through `GW1_STATIC`

## Context

At 2026/06/25 11-ish I lost connection to my home lab. This only effected one of the WAN lines, two other WANs work properly. Users using MANAGEUT to host discord/slack bots still works, due to the DNS being set to `8.8.8.8` and using `GW3` as the main gateway.
...
1. The three gateway system is awesome, however the firewall's DNS is only forwarding requests to `GW1_STATIC`, this will be changed to a new dynamic failback system.
